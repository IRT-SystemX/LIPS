...
class FullyConnectedAS(AugmentedSimulator):
    ...
    def hyperparameter_tuning(self, dataset, sizes_layer=[(150,150)], layer_act=["relu"], lr=[3e-4], batch_size=[128], epochs=[5], loss=["mse"],n_folds=3, verbose=1):
        """
        ...
        grid = GridSearchCV(estimator=model, param_grid=param_grid, n_jobs=1, cv=n_folds)
    ...
    def __create_model(self, sizes_layer=(150,150), layer_act="relu", lr=3e-4, loss="mse"):
        self.sizes_layer = sizes_layer
        self.layer_act = layer_act
        
        self.init()
        optimizer = tfko.Adam(learning_rate=lr)
        self._model.compile(optimizer=optimizer, loss=loss, metrics=["mae"])

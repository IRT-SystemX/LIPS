...
def test_fast_transform_tau():
    ...
    leap_net1._topo_vect_transformer = TopoVectTransformation(bench_config = bench_config,
                                                   params={"topo_vect_to_tau" : "given_list", "kwargs_tau" :kwargs_tau},
                                                   dataset = dataset)

    obss = leap_net1._topo_vect_transformer._make_fake_obs(dataset)
    ...
    start = time.time()
    extract_tau_bis = leap_net1._topo_vect_transformer.transform_tau_given_list(tau, leap_net1._leap_net_model.subs_index)
    end = time.time()
...
def test_fast_transform_tau_multiple_line_disconnect():
    ...
    dataset = benchmark1.train_dataset
    leap_net1._topo_vect_transformer = TopoVectTransformation(bench_config=bench_config,
                                                              params={"topo_vect_to_tau": "given_list",
                                                                      "kwargs_tau": kwargs_tau},
                                                              dataset=dataset)

    extract_tau = leap_net1._topo_vect_transformer.transform_tau_given_list(tau, leap_net1._leap_net_model.subs_index)

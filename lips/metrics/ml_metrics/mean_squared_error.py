...
def mean_squared_error(observations: dict, 
                       predictions: dict, 
                       output: str="average", 
                       squared: bool=True, 
    ...
    squared : ``bool``, optional
        Whether to square or not. If `False` Root Mean Squared Error is computed, by default ``True``
    ...
    if observations.shape != predictions.shape:
        logger.error("The observations and predictions shoud have the same shape for MSE")
        raise RuntimeError("The observations and predictions shoud have the same shape for MSE")
    ...
    if output == "raw":
        ...
        raise NotImplementedError("the output mode is unknown. Use `raw` or `average`")

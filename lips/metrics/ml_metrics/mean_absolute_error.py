...
def mean_absolute_error(observations: dict, 
                        predictions: dict, 
                        output: str="average", 
    ...
    if observations.shape != predictions.shape:
        logger.error("The observations and predictions shoud have the same shape for MAE")
        raise RuntimeError("The observations and predictions shoud have the same shape for MAE")
    
    error = np.abs(observations - predictions)
    if output == "raw":
